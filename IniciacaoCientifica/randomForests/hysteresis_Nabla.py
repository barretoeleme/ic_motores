...
model_Nabla = RandomForestRegressor()
model_Nabla.fit(X_train, y_train)

predictions = model_Nabla.predict(X_test)
print("Random forest model results in hysteresis loss for motor Nabla")
...
results = pd.DataFrame(index = newindex,
                       columns = newcolumns)
results.score.rand_for = r2_score(y_test, predictions)
results.mse.rand_for = mean_squared_error(y_test, predictions)
results.mape.rand_for = mean_absolute_percentage_error(y_test, predictions)

results.to_csv("/mnt/c/prog/IniciacaoCientifica/results/Nabla/hysteresis/results_rand_for.csv")
...
data.to_csv("/mnt/c/prog/IniciacaoCientifica/pred/Nabla/hysteresis/pred_rand_for.csv")
